...
beam_2 = ps.datautils.gaussian(shape = (20, 20), fwhm = (2, 3))
...
draw_subplot(fig, (1,2), 1, fftpack.fftshift(np.absolute(f_conv_1)), title = "Convolution function 1/1")

with np.errstate(divide='ignore', invalid='ignore'):
    f_conv = fftpack.ifft2(fft_beam_1/fft_beam_2)
draw_subplot(fig, (1,2), 2, fftpack.fftshift(np.absolute(f_conv)), title = "Convolution function 1/2")
